...
import { fetchObras, upsertObra, fetchEquipes, saveEquipes as dbSaveEquipes } from "./api";
...
export default function App() {
  ...
  const [screen, setScreen] = useState("home"); // home | calendar | equipes
  ...
  useEffect(() => {
    if (!session) { setObras([]); setEquipes([]); return; }
    let cancel = false;
    setLoading(true);
    (async () => {
      try {
        const [obs, eqs] = await Promise.all([fetchObras(), fetchEquipes()]);
        if (cancel) return;
      ...
      <div style={{ background: "#1a1a1a", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        {!selectedObra && screen !== "home" && (
          <>
            <div style={{ color: "#9ca3af", fontSize: 18, marginLeft: 4 }}>›</div>
            <div style={{ color: "#e2e8f0", fontSize: 14, fontWeight: 600 }}>
              {screen === "calendar" ? "Calendário de Obras" : "Equipes"}
            </div>
          </>
        )}
